...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* knock flash — amber wash pulses over the card when the timer asks */
  .flash {
    position: absolute;
    inset: 0;
    background: var(--knock-amber);
    opacity: 0;
    pointer-events: none;
    z-index: 20;
    border-radius: 16px;
  ...
  .flash.on {
    animation: knock-flash 0.9s ease-out;
  }
  @keyframes knock-flash {
    0%   { opacity: 0; }
    12%  { opacity: 0.28; }
    30%  { opacity: 0.05; }
    45%  { opacity: 0.28; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .flash.on { animation: knock-flash-soft 0.6s ease-out; }
    @keyframes knock-flash-soft { 0% { opacity: 0; } 40% { opacity: 0.18; } 100% { opacity: 0; } }
  }
  ...
  /* elapsed timer (hidden until begin) */
  ...
  .lane-chip {
    background: var(--pine-chip);
    color: var(--pine-chip-ink);
    font-size: 12px;
    font-weight: 600;
    padding: 4px 11px;
    border-radius: 20px;
  ...
  /* ---------- Sealed strip (internet-off reminder) ---------- */
  .sealed-strip {
    display: none;
    padding: 14px 28px;
    border-bottom: 1px solid var(--hair);
  ...
  .job .box {
    width: 22px; height: 22px;
    border: 1.5px solid var(--ink-mut);
    border-radius: 5px;
    background: var(--paper-warm);
    flex-shrink: 0;
    margin-top: 1px;
  ...
  /* check-in: always present once begun; buttons never disappear */
  ...
  /* nudge state — the timer is actively asking */
  .checkin.asking .checkin-dot {
    background: var(--clay);
    animation: pulse-ask 1.6s ease-in-out infinite;
  }
  ...
  @media (prefers-reduced-motion: reduce) {
    .checkin.asking .checkin-dot { animation: none; opacity: 1; }
  }
  ...
  /* the finish-feeling pull, shown on sort-of / no */
  ...
  .preview-link {
    ...
    cursor: pointer;
    transition: color 0.15s;
    padding: 10px 8px;   /* larger tap target — comfortable to press */
  }
  ...
  .exit-forward {
    ...
    color: var(--pine-soft);
    line-height: 1.5;
    background: var(--pine-wash, #eef4ea);
    border-radius: 9px;
    padding: 12px 14px;
    margin: 4px 0 0;
  }
...
const CONFIG = {
  ...
  scope: "9:00 – 12:00",
  mode: "sealed",                 // "sealed" | "lane"
  laneSites: [],                  // e.g. ["LinkedIn","Gmail","the 3 company sites"]
  sealedPlan: "phone in the other room",  // sealed mode only: how they're keeping the internet off ("" to hide)
  whyLabel: "Watch for",          // "Watch for" (a trap) or "Remember" (a rallying cry)
  ...
  jobsShape: "none",              // "none" | "milestones" | "checklist"
  jobs: [],                       // labels for milestones or checklist
  blockMinutes: 180,              // length of the block, in minutes
  knockMinMin: 28,
  knockMaxMin: 33,
  doneWhenShort: "1,500 new words"  // short restatement for block-end line
};
...
/* trap (why) is optional — if empty, hide the Watch-for line and the knock's quoted reason */
const hasTrap = cfg.why && cfg.why.trim().length > 0;
if (hasTrap) {
...
function updateElapsed() {
  ...
  // always show a small starting block so the bar reads as "begun," never empty
...
function showExit(endedEarly) {
  ...
  // "Block ended" if they stopped early, "Block complete" if the block ran its course
  $("exitEyebrow").textContent = endedEarly ? "Block ended" : "Block complete";
  // if the goal was a count (milestones), ask output specifically
  ...
  // Nothing is saved yet — describe what's ready, with singular/plural handling
...
// clear the invalid highlight (and warning) as soon as the user types in a flagged field
...
"",
"Then reply briefly, and output the focus file, changing ONLY the CONFIG object to match: task, scope, mode, laneSites, sealedPlan, whyLabel, why, finishFeel, jobsShape, jobs, blockMinutes, doneWhenShort. Every value from the interview. Output the complete file in one code block, then stop.",
"",
"The frozen focus-file template is included at the very bottom of this message, after the line === FOCUS FILE TEMPLATE ===. Use it exactly as-is, changing only the CONFIG object. You already have everything you need — do not ask the person to paste anything."
].join("\n");
